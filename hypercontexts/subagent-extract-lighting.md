# Hypercontext: Extract Lighting Module (Priority 2)

## Mission
Create `texture_synth/render/lighting.py` to consolidate 8+ duplicate lighting implementations.

## Current Duplicates

1. `texture_synth_v2/render_egg.py:161-182` - render_3d_egg diffuse+specular
2. `texture_synth_v2/render_egg.py:424-477` - render_pbr_surface
3. `texture_synth_v2/render_egg.py:625-707` - render_pbr_anisotropic
4. `texture_synth_v2/render_egg.py:822-875` - render_pbr_raking_light
5. `texture_synth_v2/render_egg.py:1003-1024` - render_pbr_iridescent
6. `texture_synth/render/pbr.py:354-372` - _rasterize_triangle
7. `texture_editor/render.py:216-226` - render_3d_egg
8. `lattice_extrusion_v2/render.py:69-73` - render_3d_egg_mesh

## Create: `/home/bigboi/itten/texture_synth/render/lighting.py`
...
After creating lighting.py, update render_egg.py to import these functions:

```python
from texture_synth.render.lighting import (
    lambertian_diffuse,
...
Then replace inline calculations with function calls.

## Also Extract: `/home/bigboi/itten/texture_synth/render/ray_cast.py`
...
def egg_deformation(nx, ny, z, egg_factor: float = 0.25):
...
def spherical_uv_from_ray(nx, ny, z_egg, x_egg):
    """Compute UV from ray intersection point."""
...
After refactoring:
1. Run texture_tui.py demo to verify rendering still works
2. Check that lighting looks the same as before
